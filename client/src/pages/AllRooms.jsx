...
const AllRooms = () => {
  ...
  const sortOptions=["Price: Low to High","Price: High to Low","Rating: High to Low","Rating: Low to High","Newest Listings"];
      ...
      <div>
        <div className="flex flex-col items-start text-left">
            ...
            {roomsDummyData.map((room) => (
                <div key={room._id} className="flex gap-6 flex-col md:flex-row items-start py-10 border-b border-gray-300 last:pb-30 last:border-0">
                    <img onClick={()=>{navigate(`/rooms/${room._id}`);scrollTo(0,0)}}
                    src={room.images[0]} alt="hotel-img" title="View Room Details" className="max-h-full md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer"/>
